<template>
  <div :class="['sidebar-scroll', isZoom ? 'is-zoom' : '']">
    ...
    <div v-if="mode === 'list'">
      <div :class="activeId === item.id ? 'nav-box-active' : 'nav-box'" v-for="item in sidebarList" :key="item.id">
        <router-link class="nav-box-title" tag="div" :to="item.link">
          <img v-if="item.icon || item.disableIcon" :src="activeId === item.id ? item.icon : item.disableIcon"
            width="25px" height="25px" />
          <span :class="['nav-box-tip', isZoom ? 'is-zoom' : '']" v-show="!isZoom">{{ item.name }}</span>
        </router-link>
      </div>
    </div>
